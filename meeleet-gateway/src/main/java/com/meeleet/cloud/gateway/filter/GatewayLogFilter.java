    ...
    public Mono<Void> writeLog(ServerWebExchange exchange, GatewayFilterChain chain, TraceLog traceLog) {
        ...
        return chain.filter(exchange.mutate().response(serverHttpResponseDecorator)
                ...
                .then(Mono.fromRunnable(
                        () -> {
                            traceLog.setResponseStatusCode(exchange.getResponse().getStatusCode().value());
                            log.info(traceLog.toResponseString());
                        }
                ));
    }
        ...
        return new ServerHttpResponseDecorator(response) {
            ...
            public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
                if (body instanceof Flux) {
                    ...
                    long executeTime = DateUtil.betweenMs(DateUtil.parse(traceLog.getRequestTime(), DatePattern.NORM_DATETIME_MS_PATTERN), now);
